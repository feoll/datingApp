...
class FakeSwipeRepository : SwipeRepository {
    override suspend fun getProfileSwipeList(profileId: Int): Resource<List<ProfileCard>> {
        ...
        for (i in 1..9) {
            list.add(
                ProfileCard(
                    i, i, 178, "Female", "Алина",
                    18, "Звезда кантри-музыки. Обожаю гулять вечером по парку. Занимаюсь спортом и программирую на Sping Java. Программирую примерно 40 лет потому что потому, хе-хе", "Беларусь", "Минск",
